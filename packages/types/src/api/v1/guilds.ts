...
export const UpdateGuildRequestSchema = z.object({ name: z.string().min(1).optional(), memberCount: z.number().int().min(0).optional(), prefix: z.string().max(5).regex(/^\S+$/).optional(), isActive: z.boolean().optional() });

export const GuildParamsSchema = z.object({ id: z.string().min(1, "Guild ID is required") });
...
export const validateCreateGuildRequest = (data: unknown): CreateGuildRequest => CreateGuildRequestSchema.parse(data);

export const validateUpdateGuildRequest = (data: unknown): UpdateGuildRequest => UpdateGuildRequestSchema.parse(data);
