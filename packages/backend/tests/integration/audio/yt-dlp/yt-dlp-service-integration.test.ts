...
describe.concurrent("YtDlpService Integration Tests", () => {
    ...
    it.skipIf(process.env.CI)("should extract audio from a real YouTube URL", testExtractAudioFromYouTubeUrl, 30000);
    async function testExtractAudioFromYouTubeUrl() {
    ...
    it.skipIf(process.env.CI)("should get video info from a real YouTube URL", testGetVideoInfoFromYouTubeUrl, 15000);
    async function testGetVideoInfoFromYouTubeUrl() {
        // Check if yt-dlp is available first
        if (!(await isYtDlpAvailable())) {
            console.log("⏭️ Skipping test: yt-dlp not available");
            return;
    ...
    it("should handle invalid URLs gracefully", testHandleInvalidUrls, 15000);
    async function testHandleInvalidUrls() {
    ...
    it("should handle malformed URLs", testHandleMalformedUrls, 15000);
    async function testHandleMalformedUrls() {
