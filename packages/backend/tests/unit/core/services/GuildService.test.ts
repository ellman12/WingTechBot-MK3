...
vi.mock('@adapters/repositories/KyselyGuildRepository.js', () => ({
  findGuildById: vi.fn(),
  findAllGuilds: vi.fn(),
  createGuildInDb: vi.fn(),
  updateGuildInDb: vi.fn(),
  deleteGuildFromDb: vi.fn(),
  guildExists: vi.fn(),
}));
...
describe('GuildService', () => {
  ...
  describe('createGuild', () => {
    it('should create guild successfully', async () => {
      const createData: CreateGuildData = {
        id: 'new-guild-id',
        name: 'New Guild',
        ownerId: 'owner-123',
        memberCount: 0,
    ...
    it('should throw error when guild already exists', async () => {
      ...
      await expect(createGuild(mockDb, createData)).rejects.toThrow(
        'Guild with id existing-guild-id already exists'
      );
    });
