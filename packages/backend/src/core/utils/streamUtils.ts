...
export const createPreBufferedStream = async (sourceStream: Readable, sourceName: string, abortSignal?: AbortSignal): Promise<Readable> => {
    logger.debug(`[SoundService] Creating pre-buffered stream for: ${sourceName}`);
    ...
    return new Promise((resolve, reject) => {
        ...
        const abortHandler = () => {
            if (!isResolved) {
                logger.debug(`[SoundService] Pre-buffering aborted for: ${sourceName}`);
                sourceStream.destroy();
                bufferedStream.destroy();
                isResolved = true;
                reject(new Error("Pre-buffering aborted"));
        ...
        const onData = (chunk: Buffer) => {
            ...
            if (!isResolved && bytesBuffered >= preBufferTarget) {
                ...
                logger.debug(`[SoundService] Pre-buffer target reached for ${sourceName}: ${bytesBuffered} bytes in ${prebufferTime}ms`);
                isResolved = true;
        ...
        const onEnd = () => {
            logger.debug(`[SoundService] Source stream ended for: ${sourceName}, total bytes: ${bytesBuffered}`);
            if (!bufferedStream.destroyed) {
                bufferedStream.end();
            }
            ...
            if (!isResolved) {
                logger.debug(`[SoundService] Stream ended before pre-buffer target, resolving with ${bytesBuffered} bytes`);
                isResolved = true;
                abortSignal?.removeEventListener("abort", abortHandler);
                resolve(bufferedStream);
            }
        ...
        const onError = (error: Error) => {
            logger.error(`[SoundService] Source stream error for ${sourceName}:`, error);
        ...
        const timeout = setTimeout(() => {
            if (!isResolved) {
                logger.debug(`[SoundService] Pre-buffer timeout for ${sourceName}, resolving with ${bytesBuffered} bytes`);
                isResolved = true;
                abortSignal?.removeEventListener("abort", abortHandler);
                resolve(bufferedStream);
            }
