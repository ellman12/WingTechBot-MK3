...
import { validateCreateGuildRequest, validateGuildParams, validateUpdateGuildRequest } from "@wingtechbot-mk3/types/api/v1/guilds";
import type { Request, RequestHandler, Response } from "express";
import type { Kysely } from "kysely";
import { z } from "zod/v4";

import { GuildNotFoundError } from "../../../../core/errors/GuildErrors.js";
import { createGuild, deleteGuild, getAllGuilds, getGuildById, updateGuild } from "../../../../core/services/GuildService.js";
...
const handleError = (error: unknown, res: Response): void => {
    if (error instanceof GuildNotFoundError) {
        const errorResponse: ApiErrorResponse = { success: false, error: error.message };
        res.status(404).json(errorResponse);
        return;
    }

    if (error instanceof z.ZodError) {
        const errorResponse: ApiErrorResponse = { success: false, error: "Validation failed", details: error.issues.map(issue => ({ path: issue.path.join("."), message: issue.message })) };
    ...
    const errorResponse: ApiErrorResponse = { success: false, error: "Internal server error" };
...
export const getGuildsV1Handler =
    ...
    async (_req: Request, res: Response): Promise<void> => {
        try {
            ...
            res.json({ success: true, data: apiGuilds });
        } catch (error) {
...
export const getGuildByIdV1Handler =
    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            res.json({ success: true, data: apiGuild });
        } catch (error) {
            handleError(error, res);
        }
    };
...
export const createGuildV1Handler =
    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const guildData = { id: requestData.id, name: requestData.name, ownerId: requestData.ownerId, memberCount: requestData.memberCount ?? 0, prefix: requestData.prefix ?? "!", isActive: true };

            const guild = await createGuild(db, guildData);
            const apiGuild = GuildPresenter.toApiV1(guild);
            res.status(201).json({ success: true, data: apiGuild });
        } catch (error) {
            handleError(error, res);
        }
    };
    ...
    async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (Object.keys(updateData).length === 0) {
                const errorResponse: ApiErrorResponse = { success: false, error: "Update data is required" };
                res.status(400).json(errorResponse);
                return;
            }
